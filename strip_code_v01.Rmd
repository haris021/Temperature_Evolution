...
```{r colors}
txt_clr <- "white"
pal1 <- c("#105182", "#1a7bc5", "#42a2f1", "#E9F1F2", "#ff9193", "#f1434a", "#c91022", "#8d0613", "#4D030A","#8d0613", "#4D030A")
```

```{r monthly_average}
monthly <- climate %>%
  filter(Country == "Pakistan", !is.na(AverageTemperature)) %>%
...
axis_labels <- tibble(month = lubridate::month(seq(1,12,1), label = TRUE),
                      pos = seq(1,12,1))
seg <- tibble(x = c(0, 0, 10, 0, 9, 3, 8, 5, 6, 8,8),
              xend = c(12.5, 3, 12.5, 5, 12.5, 6, 11, 10, 8,6,6),
              y = c(0, 5, 5, 10, 10, 15, 15, 20,20, 25,30),
              yend = c(0, 5, 5, 10, 10, 15, 15, 20,20, 25, 30))
seg_lab <- tibble(x = c(0, 0, 0, 2, 5, 6,6.5),
                  y = seq(0,30, 5))
monthly_plt <- ggplot() +
  ...
  scale_y_continuous(limits = c(-4,31), breaks = seq(0,30,5)) +
...
final <- monthly_plt / (yearly_plt + inset_element(legend, 0.2, 0.1, 0.8, 0.27, align_to = "full")) + plot_layout(heights = c(5,1)) +
  plot_annotation(
    title = "Temperature evolution in Pakistan",
      ...
      caption = "RethinkingIndus | Viz: Haris Mushtaq | Data: Berkeley Earth / Kaggle",
      theme = theme(
...
ragg::agg_png(here::here("render", paste0("Pakistan_updated", format(Sys.time(), "%Y%m%d_%H%M%S"), ".png")), res = 320, width = 10, height = 10, units = "in")
final
